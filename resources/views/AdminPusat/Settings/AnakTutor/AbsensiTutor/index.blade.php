                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="absen-table" class="display table table-striped table-hover">
                                ...
                                <tbody>
                                    @foreach($data_absensi_tutor as $absen)
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $absen['nama_tutor'] }}</td>
                                        <td>
                                            <div class="form-button-action">
                                                <a href="{{ route('absen_tutor.edit', $absen['id']) }}" class="btn btn-link btn-primary btn-lg edit-btn">
        ...
        var table = $('#absen-table').DataTable({
            ...
            "drawCallback": function(settings) {
                $('.delete-btn').on('click', function() {
                    ...
                    $('#deleteAbsensiForm').attr('action', '/admin_pusat/settings/absensi_user_tutor/' + id);
                });

                $('.edit-btn').on('click', function() {
                    let id = $(this).data('id');
                    let currentPage = table.page.info().page;
                    window.location.href = '/admin_pusat/settings/absensi_user_tutor/' + id + '/edit?current_page=' + currentPage;
                });
            }
        });
