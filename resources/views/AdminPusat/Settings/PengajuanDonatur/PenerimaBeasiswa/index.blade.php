...
@section('content')
    <div class="container">
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Data Pengajuan Donatur CPB</h4>
                        </div>
                        <div class="card-body">
                                ...
                                <table id="penerima-table" class="display table table-striped table-hover">
                                    <thead>
                                        <tr>
                                            ...
                                            <th><input type="text" id="search-nama" class="form-control"
                                                    placeholder="Cari Nama"></th>
                                            <th><input type="text" id="search-agama" class="form-control"
                                                    placeholder="Cari Agama"></th>
                                            <th><input type="text" id="search-status_cpb" class="form-control"
                                                    placeholder="Cari Status"></th>
                                            <th><input type="text" id="search-jk" class="form-control"
                                                    placeholder="Cari JK"></th>
                                            <th><input type="text" id="search-anak_ke" class="form-control"
                                                    placeholder="Cari Anak Ke"></th>
                                            <th><input type="text" id="search-survey" class="form-control"
                                                    placeholder="Cari Hasil Survey"></th>
                                            <th><input type="text" id="search-donatur" class="form-control"
                                                    placeholder="Cari Donatur"></th>
                                            <th><input type="text" id="search-action" class="form-control"
                                                    placeholder="Cari Action"></th>
                                        </tr>
        ...
        <!-- Modal Pilih Donatur -->
        <div class="modal fade" id="donaturModal" tabindex="-1" role="dialog" aria-labelledby="donaturModalLabel"
            aria-hidden="true">
...
@section('scripts')
    <script>
        $(document).ready(function() {
            const STATUS_CPB_PB =
                'PB'; // Set status PB secara manual karena tidak bisa diambil langsung dari PHP dalam JavaScript

            // Inisialisasi DataTable dengan filter dan pencarian per kolom
            ...
            // Filter Wilayah Binaan dan Shelter
            ...
            // Handle modal untuk konfirmasi penghapusan donatur
            $('#confirmDeleteModal').on('show.bs.modal', function(event) {
                var button = $(event.relatedTarget);
                var idAnak = button.data('id');
                var namaAnak = button.data('nama');
                $('#delete-nama').text(namaAnak);
                $('#deleteDonaturForm').attr('action', '/admin_pusat/settings/pengajuan_donatur/' + idAnak);
            });

            // Modal Pilih Donatur dengan action dinamis
            $('#donaturModal').on('show.bs.modal', function(event) {
                var button = $(event.relatedTarget);
                var idAnak = button.data('id');
                var formAction = "{{ route('pengajuan_donatur.update', '') }}/" + idAnak;
                $('#formPilihDonatur').attr('action', formAction);
            });

            // Filter khusus status CPB menggunakan konstanta dan id yang tepat
            $('input#search-status_cpb').on('keyup change', function() { // Pastikan id di sini sama dengan HTML
                var searchTerm = this.value.trim().toUpperCase();

                // Debugging log untuk memastikan input
                console.log("Input Search Status:", searchTerm);

                if (searchTerm === 'PB') {
                    table.column(3).search('PB').draw();
                } else if (searchTerm === 'CPB') {
                    table.column(3).search('CPB').draw();
                } else {
